...
The question is: could it possibly be, instead, that this slight wobble of the Sun is juat a direct consequence of the influence of Mars, its binary companion?

In any event, such oscillations on the part of host stars in binary systems are precisely what our modern-day astronomers look for (using sophisticated spectrometers and assorted state-of-the-art techniques) when trying to determine if a given star may host a smaller binary companion. In light of this, it would therefore seem quite reasonable to assume that the Sun’s small oscillation around its nucleus is caused by none other than its binary companion, Mars.
