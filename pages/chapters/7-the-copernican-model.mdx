...
As you can see, the TYCHOS model has Venus returning to the same line of sight between Earth and star Regulus in 816 days. In any case, this is precisely what the Tychosium 3D simulator shows. Again, _you_ (the reader) are the judge as to which model makes more sense: the Copernican model has Venus returning facing the same star after 816 days (even though during this time period Earth & Venus would have moved 'sideways' by 200Mkm) whereas the TYCHOS model has Venus naturally reconjuncting with that same star - simply because it returns to the very same celestial longitude on both occasions!      
